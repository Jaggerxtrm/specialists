...
16. Monitor filters for `sp log --json` read `event_family` / `event_name` / `severity` from `row.forensic_event`, never top level — top-level access silently matches nothing and coordinators read the ensuing silence as "still running". Terminal `event_name`s are exactly `job.completed | job.failed | job.cancelled`; `waiting` is a keep-alive-between-turns state, NOT terminal. Full recipes, the row envelope, and the 0-tool-calls-on-terminal sanity check live in [references/monitoring.md](references/monitoring.md).
