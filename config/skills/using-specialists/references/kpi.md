...
prompt payload, usage trajectories, model/role outliers, or regressions in execution
...
4. first-turn and cumulative model usage;
5. prompt/skill/rule payload size;
6. turns and tool-call distribution;
7. model/role outliers and variance.

API/provider usage telemetry is authoritative when available. Payload-component events or
file-size approximations explain cost but are not exact billing truth.
...
and usage cost, failure/stall/waiting outliers, context-size outliers, likely cause with
