...
Targeted lookup replaces any bulk dump — in addition to the canonical skill's session-start lookup (`bd ready` / `bd search`, `bv --robot-triage` only when graph-aware ranking is needed):
