...
| Section | Summary |
|---|---|
| [Core commands](#core-commands) | | Command | Purpose | |
| [`specialists run`](#specialists-run) | Tracked work: |
| [`specialists feed`](#specialists-feed) | Send a mid-run steering message to a running background job |
| [`specialists steer`](#specialists-steer) | Send a mid-run steering message to a running background job |
...
Diagnoses the installation and runtime environment. Checks:

- `pi`, `bd`, `xt` binaries present and functioning
- Claude Code hooks registered in `.claude/settings.json`
- Both bundled hooks present at `.specialists/default/hooks/`:
  - `specialists-complete.mjs`
  - `specialists-session-start.mjs`
- MCP server entry in `.mcp.json`
- Required directories (`.specialists/default/`, `.specialists/jobs/`) exist

Run after `specialists init` or when hooks appear not to fire.
