...
| `tests/unit/specialist/runner.test.ts` | ~~2/54~~ **restored 2026-08-14** | (was) `expected "vi.fn()" to not be called at all, but actually been called 1 times` | Root cause was `classifyFallbackError` rate-limit branch not recognizing NIM `ResourceExhausted` / quota text (only the regex `/rate.?limit|429|too many requests/`), so `fallback_step` telemetry mislabeled rate-limit failures as `transient`. Fixed by routing the rate-limit branch through `isRateLimitError` (unitAI-ip5jz). File removed from `vitest.config.ts` quarantine list. |
