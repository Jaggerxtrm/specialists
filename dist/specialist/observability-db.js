...
const DEFAULT_DB_DIRECTORY_RELATIVE_TO_GIT_ROOT = ['.specialists', 'db'];
export const OBSERVABILITY_SCHEMA_VERSION = 10;
...
export function resolveObservabilityDbLocation(cwd = process.cwd()) {
...
export function ensureObservabilityDbFile(location) {
...
export function ensureGitignoreHasObservabilityDbEntries(gitRoot) {
...
export function isObservabilityDbInitialized(location) {
    if (!existsSync(location.dbPath) || !hasSqlite3Binary())
        return false;
...
export function isPathInsideJobsDirectory(pathToCheck, gitRoot) {
    const jobsDirPrefix = `${join(gitRoot, '.specialists', 'jobs')}${sep}`;
    return pathToCheck.startsWith(jobsDirPrefix);
}
//# sourceMappingURL=observability-db.js.map