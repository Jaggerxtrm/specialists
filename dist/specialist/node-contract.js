...
const PHASE_KIND_VALUES = ['explore', 'design', 'impl', 'review', 'fix', 're_review', 'custom'];
export const BARRIER_TYPES = ['all_members_terminal'];
export const NODE_COMPLETION_STRATEGIES = ['pr', 'manual'];
...
export function renderForSystemPrompt() {
    return [
        ...
        '- After each wait-phase barrier, read participating member results with `sp result $SPECIALISTS_NODE_ID:<member-key> --wait --json`, synthesize the evidence, then decide the next phase or remain in waiting.',
...
export function renderForFirstTurnContext(ctx) {
    return [
...
export function renderForResumePayload(update) {
    return [
        'node_resume_payload:',
        renderJsonSnippet({
...
export function renderForDocs() {
    ...
    return [
        ...
        '- After `wait-phase` completes, read every participating member result with `sp result $SPECIALISTS_NODE_ID:<member-key> --wait --json`, synthesize the evidence, then decide the next phase or remain in waiting for operator closure.',
        '',
        '### Phase kinds',
        phaseDocs,
        '',
