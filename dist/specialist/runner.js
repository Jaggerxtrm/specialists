...
function isToolAvailable(tool, permissionLevel) {
    ...
    if (!gatedLevels)
        return true; // not gated — available at all levels (read, grep, find, ls, glob, notebook, etc.)
...
const OUTPUT_TYPE_SCHEMA_EXTENSIONS = {
    codegen: {
        type: 'object',
        properties: {
...
function validateOutputContract(output, responseFormat, outputSchema) {
    ...
    if (schemaErrors.length > 0) {
        warnings.push(`Output contract warning: schema mismatch (${schemaErrors.length} issue${schemaErrors.length === 1 ? '' : 's'}).`, ...schemaErrors.map(issue => `  - ${issue}`));
    }
    return warnings;
}
export class SpecialistRunner {
    ...
    async run(options, onProgress, onEvent, onMetric, onMeta, onKillRegistered, onBeadCreated, onSteerRegistered, onResumeReady, onToolStartCallback, onToolEndCallback) {
        ...
        const preResults = preScripts
            .map(s => runScript(s.run ?? s.path, runCwd))
...
---\n`;
        ...
        let output;
        let sessionBackend = model; // captured before kill() can destroy meta
        let runMetrics;
        let session;
        ...
        try {
            ...
            if (resolvedNodeId)
                envVars.SPECIALISTS_NODE_ID = resolvedNodeId;
            if (options.variables?.SPECIALISTS_JOB_ID)
                envVars.SPECIALISTS_JOB_ID = options.variables.SPECIALISTS_JOB_ID;
     ...
     */
    async startAsync(options, registry) {
        ...
        this.run(options, (text) => registry.appendOutput(jobId, text), (eventType) => registry.setCurrentEvent(jobId, eventType), undefined, (meta) => registry.setMeta(jobId, meta), (killFn) => registry.setKillFn(jobId, killFn), (beadId) => registry.setBeadId(jobId, beadId), (steerFn) => registry.setSteerFn(jobId, steerFn), (resumeFn, closeFn) => registry.setResumeFn(jobId, resumeFn, closeFn))
