...
import { formatEventInlineDebounced } from './format-helpers.js';
...
async function parseArgs(argv) {
    ...
    if (!name || name.startsWith('--')) {
        console.error('Usage: specialists|sp run <name> [--prompt "..."] [--bead <id>] ' +
    ...
    for (let i = 1; i < argv.length; i++) {
        ...
        if (token === '--no-worktree') {
            console.error('Error: --no-worktree has been removed. ' +
...
function resolveWorkingDirectory(args, jobsDir, permissionRequired, readStatus) {
    ...
    if (args.reuseJobId !== undefined) {
        ...
        if (!targetStatus) {
            console.error(`Error: cannot read status for job '${args.reuseJobId}'. ` +
        ...
        if (editCapable && !args.forceJob && isBlockedStatus) {
            console.error(`Target job ${args.reuseJobId} is still running (status: ${targetJobStatus}). ` +
        ...
        if (shouldBlockUnknownStatus) {
            console.error(`Target job ${args.reuseJobId} has unknown status '${String(targetJobStatus)}'. ` +
...
export async function run() {
    ...
    const perm = permission === 'LOW' || permission === 'MEDIUM' || permission === 'HIGH'
        ? permission
        : 'READ_ONLY';
    ...
    }, jobsDir, perm, (jobId) => statusReader.readStatus(jobId));
