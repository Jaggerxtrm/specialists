...
export function buildSessionName(specialist, suffix) {
 ...
 */
export function createTmuxSession(name, cwd, cmd, extraEnv = {}) {
    const exports = [
    ...
    const result = spawnSync('tmux', ['new-session', '-d', '-s', name, '-c', cwd, wrappedCommand], { encoding: 'utf8', stdio: 'pipe' });
    if (result.status !== 0) {
