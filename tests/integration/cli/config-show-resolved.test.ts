...
// Source bug: `config show <spec> --resolved` reports package canonical manifest,
// not globally overlaid manifest, for Phase 1 user.json fields.
// Follow-up bead will be filed by orchestrator. Flip describe.skip -> describe after fix lands.
...
describe.skip('integration: config show --resolved global Phase 1 overlay', () => {
  let tempHome = '';
  ...
  it('shows execution.extensions.serena override in effective manifest', async () => {
    tempHome = await mkdtemp(join(tmpdir(), 'specialists-config-show-'));
    ...
    const manifest = parseEffectiveManifest(result.stdout) as { specialist: { execution: { extensions?: { serena?: boolean } } } };
    expect(manifest.specialist.execution.extensions?.serena).toBe(false);
  });

  it('shows notes_mode override in effective manifest', async () => {
    tempHome = await mkdtemp(join(tmpdir(), 'specialists-config-show-'));
    originalHome = process.env.HOME;
    originalXdg = process.env.XDG_CONFIG_HOME;
    process.env.HOME = tempHome;
    ...
    expect(result.status).toBe(0);
    const manifest = parseEffectiveManifest(result.stdout) as { specialist: { notes_mode?: string } };
    expect(manifest.specialist.notes_mode).toBe('final-only');
  });
});
