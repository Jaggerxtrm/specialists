...
| Section | Summary |
|---|---|
| [Starting RPC Mode](#starting-rpc-mode) | Common options: |
| [Protocol Overview](#protocol-overview) | - **Commands**: JSON objects sent to stdin, one per line |
| [Commands](#commands) | Send a user prompt to the agent |
| [Events](#events) | Events are streamed to stdout as JSON lines during agent operation |
| [Extension UI Protocol](#extension-ui-protocol) | Extensions can request user interaction via `ctx |
| [Error Handling](#error-handling) | Failed commands return a response with `success: false`: |
| [Types](#types) | Source files: |
| [Example: Basic Client (Python)](#example-basic-client-python) | See [`test/rpc-example |
| [Example: Interactive Client (Node.js)](#example-interactive-client-nodejs) | See [`test/rpc-example |
<!-- END INDEX -->
