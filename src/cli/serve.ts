...
function parseArgs(argv: string[]): ServeArgs {
  ...
  return { port, concurrency, shutdownGraceMs, userDir, fallbackModel };
}
...
export async function startServe(argv: string[] = process.argv.slice(3)) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (active >= args.concurrency) return sendJson(res, 429, { success: false, error: 'too_many_requests', error_type: 'quota' });
    ...
    const work = (async () => {
      try {
        ...
        const result = await runScriptSpecialist(parsed, { loader, fallbackModel: args.fallbackModel });
