...
export function renderForSystemPrompt(): string {
  return [
    ...
    '- After each wait-phase barrier, read participating member results with `sp result --node ... --member ... --wait --json`, synthesize the evidence, then decide the next phase or remain in waiting.',
...
export function renderForDocs(): string {
  ...
  return [
    ...
    '- After `wait-phase` completes, read every participating member result with `sp result --node ... --member ... --wait --json`, synthesize the evidence, then decide the next phase or remain in waiting for operator closure.',
    '',
    '### Phase kinds',
    phaseDocs,
    '',
