...
- **Node coordination architecture (unitAI-3f7b)** → Coordinator READ_ONLY emitting typed actions; NodeSupervisor is effect executor; orchestrator owns FIFO merge; SSoT is src/specialist/node-contract.ts (Zod schema + phase_kind + action vocabulary + state machine); flattened node operations now route through top-level CLI (`sp ps`, `sp feed --node`, `sp steer <coordinator-job-id>`, `sp attach <coordinator-job-id>`, `sp result --node <id> --member <key>`)
