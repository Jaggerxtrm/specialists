...
- **Node coordination (unitAI-3f7b + unitAI-t4ss + unitAI-8zui)** → Coordinator is LOW-permission CLI-native orchestrator (bash-only); NodeSupervisor is effect executor; SSoT is src/specialist/node-contract.ts; flattened node operations route through top-level CLI (`sp ps --node`, `sp feed --node`, `sp steer <coordinator-job-id>`, `sp attach <coordinator-job-id>`, `sp result --node <id> --member <key>`); coordinator MUST read member results before synthesis
